...
4. Migration rate (*M*) is in units of 4*Nm*, where *m* equals equals the number of migrants per generation.

Zero, one, or more free parameter values can be established for each parameter type within a model, where parameter indeces are labeled consecutivley as 1, 2,...*K* for *K* free parameters for a given type of parameter. For example, if the `collapseMatrix` is 
