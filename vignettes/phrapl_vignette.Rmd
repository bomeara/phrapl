...
Then, to extract the files, in the terminal, navigate to the ms directory and type

    tar -xvf ms.gz

To compile the program (per the ms manual), navigate to msdir (by typing `cd msdir`) and type
...
The scripting language, Perl, comes pre-installed on Mac OS X systems. However, to make sure, type `perl -v` in the terminal, which will give the version installed. If a new version needs to be installed for whatever reason, you can download it from the Perl website:
...
PHRAPL uses functions from a variety of other R packages. To install these, open R and load the PHRAPL library by typing `library(phrapl)`. Then type
