...
plotModel<-function(migrationIndividual, parameterVector=NULL, taxonNames=NULL) {
	open3d()
	...
	for (regime in sequence(dim(collapseMatrix)[2])) {
		...
		for (i in sequence(dim(collapseMatrix)[1])) {
			if (!is.na(collapseMatrix[i, regime])) {
				...
				shade3d(addNormals(subdivision3d(cylinder3d(center, radius=base.radius* n0multiplierParameters[n0multiplierMap[i, regime] ], sides=8, twist=1, closed=-2), depth=2)), col=c("gray",rev(brewer.pal(max(3,length(parameterVector)),"Set1")))[n0multiplierMap[i, regime]])
			}
		}
					...
					if(from.index != to.index) {
						if(!is.na(local.migrationMatrix[from.index, to.index])) {
							if(local.migrationMatrix[from.index, to.index]>0) {
								...
								arrow3d2(base=c(current.terminal.pos[from.index,], regime+offset), tip=c(current.terminal.pos[to.index,],regime+offset),rad=migrationParameters[local.migrationMatrix[from.index, to.index] ]*base.radius/3, col=(brewer.pal(max(3,length(parameterVector)),"Set1"))[local.migrationMatrix[from.index, to.index]])
							}	
						}	
					}
				} 	
